...
<template>
  <div class="bg-white dark:bg-theme-midnight rounded-2xl shadow-lg flex flex-col gap-4">
    <div class="flex flex-col gap-2 grow overflow-y-auto px-4 pt-4">
      <ChatBubble v-for="message in chatStore.messages" :key="message.id" :message="message.content" :type="message.role" />
    </div>
    <div class="mx-4 mb-4 relative flex">
      ...
      <button
        @click="chatStore.addUserMessage()" :disabled="!chatStore.userInput.trim()"
        class="absolute top-1/2 right-4 -translate-y-1/2 p-2 rounded-2xl shadow-lg bg-theme-seablue text-white cursor-pointer 
